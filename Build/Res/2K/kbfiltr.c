...
--*/
{
    ...
    //Save the registry path.  We can use it later to get config settings
    MyRegistryPath.MaximumLength = RegistryPath->Length + sizeof(UNICODE_NULL);
    MyRegistryPath.Length = RegistryPath->Length;
    MyRegistryPath.Buffer = ExAllocatePool(PagedPool, MyRegistryPath.MaximumLength);
    if (!RegistryPath->Buffer) {
       DebugPrint(("Could not allocate pool for registry path\n"));
       return STATUS_INSUFFICIENT_RESOURCES;
    }
    RtlCopyUnicodeString(&MyRegistryPath, RegistryPath);
    //
...
--*/
{
    ...
    //
    case IOCTL_INTERNAL_I8042_HOOK_KEYBOARD: //Not needed for upper filter driver
    //
...
--*/
{
    ...
    CheckKeys2(InputDataStart, InputDataEnd);
