...
uses
  ...
  madCodeHook,
  SysUtils,
  madStrings, {MemMappedFile,} PrinterIPC,
  CommonPrintMonitor;
...
var
  ...
  //mh 6/16/2010
  IsPS, IsIE8Job: Boolean;

function IsPostScriptPrinter(dc : hdc) : boolean;
var
  TestInt : integer;
...
begin
  ...
  Dlog('Allow Page - StartPageNext Result = ' + IntToStr(Result));
  if Result <= 0 then
    Dlog('Error Returned:' + SysErrorMessage(GetLastError));
...
begin
  ...
  //mh 6/16/2010
  if IsPS and IsIE8Job and
     (not GetRejectingPages) then
  begin
    //Since EndDocNext doesn't force an EndPage with a PostScript Driver here,
    //manually send the EndPage, then immediately Cancel the pending operation.
    //Issuing the EndDocNext afterwards seems to work with one problem - it
    //counts another page as having been printed instead of rejected.  Adjust for
    //those page counts in EndPageCallBack.
    //
    //(Note: Everything else tried here had resulted in an IE script error:
    // res://iframe.dll/preview.js)
    Result := EndPage(dc);
    Dlog('IE8Job/PS - Reject Page - EndPage Result = ' + IntToStr(Result));
    CancelDC(dc);
    Result := EndDocNext(dc);
    Dlog('IE8Job/PS - Reject Page - EndDocNext Result = ' + IntToStr(Result));

  end else begin
    Result := EndDocNext(dc);
    Dlog('Rejecting Page - EndDocNext Result = ' + IntToStr(Result));
    if Result <= 0 then
      Dlog('Error Returned: ' + SysErrorMessage(GetLastError));
  end;
...
begin
  ...
  try
    ...
    DLog('    Pages Rejected = ' + IntToStr(GetPagesRejected));
    ...
    begin
      ...
      // At least 89087909805 one more page can be printed
...
begin
  ...
  //mh 4/14/2009 - added following pause for XP SP3 printing problem - paCreateDC not being processed
  // from web browser printing.  OK for all other versions and SPs
  sleep(200);
...
begin
  ...
  IsIE8Job := (0 <> GetRegistryInt(HKEY_CURRENT_USER, SERVICE_PRINTER_IPC_REG_KEY, 'IsIE8Job', 0));


  Dlog('----------------------');
  if IsPS then
    Dlog('POSTSCRIPT Printer')
  else
    Dlog('NON-POSTSCRIPT Printer');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //mh 6/18/2010 - Adjust Printed/Rejected page counts/ for ie8/postscript jobs
        //               to account for last allowed page not triggering EndPage normally
        //========================================================================================
        ...
        begin
          SetNumberOfPagesPrinted(0);
          SetNumberOfPagesRejected(1);
        end;

        if LAllowed > 0 then
        begin
          LCopies := GetNumberOfCopies;
          if (LCopies = 0) then LCopies := 1;
          if IsPS and
             IsIE8Job and
             ((GetNumberOfPagesPrinted div LCopies) = (LAllowed div LCopies)) and
             (GetNumberOfPagesRejected = 0) then
          begin
