...
type
  TfmMain = class(TForm)
    ...
    procedure ckbIE8JobClick(Sender: TObject);
  private
    { Private declarations }
    FPipe: TPipeServer;
    function GetISIE8Job: Boolean;
    procedure SetISIE8Job(AValue: Boolean);
...
begin
  ...
  begin
    try
      ...
      begin
        if ValueExists('IsIE8Job') then
          Result := (0 <> ReadInteger('IsIE8Job'))
        else
          Result := FALSE;
      end;
    finally
...
procedure TfmMain.SetISIE8Job(AValue: Boolean);
begin
  with TRegistry.Create do
  begin
    try
      RootKey := HKEY_CURRENT_USER;
      if OpenKey(SERVICE_PRINTER_IPC_REG_KEY, TRUE) then
      begin
        if AValue then
          WriteInteger('IsIE8Job', 1)
        else
          WriteInteger('IsIE8Job', 0);
      end;
    finally
      Free;
    end;
...
procedure TfmMain.ckbIE8JobClick(Sender: TObject);
begin
  SetIsIE8Job(ckbIE8Job.Checked);
end;
